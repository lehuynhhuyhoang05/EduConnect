...
export function ClassDetailPage() {
  ...
  const { data: assignmentsData } = useAssignmentsByClass(classId || '');
  const { data: sessionsData } = useLiveSessionsByClass(classId || '');
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 text-xs text-neutral-500 dark:text-neutral-400">
                          <span className="flex items-center gap-1">
                            ...
                            {format(new Date(session.scheduledAt || session.startTime || new Date()), 'dd/MM')}
                          </span>
                          <span className="flex items-center gap-1">
                            <Clock className="w-3 h-3" />
                            {format(new Date(session.scheduledAt || session.startTime || new Date()), 'HH:mm')}
                          </span>
                        </div>
                      </div>
                    ))}
        ...
        {activeTab === 'members' && (
          <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-soft p-6">
            ...
            <div className="space-y-3">
              ...
              {classData.teacher && (
                <div className="flex items-center gap-3 p-3 bg-primary-50 dark:bg-primary-500/10 rounded-xl">
                  <div className="w-10 h-10 rounded-full bg-primary-500 flex items-center justify-center text-white font-semibold">
                    {(classData.teacher.fullName || classData.teacher.name || '?')[0]}
                  </div>
                  <div className="flex-1">
              ...
              {/* Students */}
              {members?.filter((m: any) => m.userId !== classData.teacherId).map((member: any) => (
                <div key={member.id} className="flex items-center gap-3 p-3 bg-neutral-50 dark:bg-neutral-700 rounded-xl">
                  <div className="w-10 h-10 rounded-full bg-neutral-300 dark:bg-neutral-600 flex items-center justify-center text-neutral-700 dark:text-neutral-300 font-semibold">
                    {(member.user?.name || member.name)?.[0] || 'U'}
                  </div>
                  <div className="flex-1">
                    <p className="font-medium text-neutral-900 dark:text-white">
