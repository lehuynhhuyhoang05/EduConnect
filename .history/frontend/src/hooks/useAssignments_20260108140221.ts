...
export const assignmentKeys = {
  ...
  byClass: (classId: string) => [...assignmentKeys.all, 'byClass', classId] as const,
  submissions: (assignmentId: string) => [...assignmentKeys.all, 'submissions', assignmentId] as const,
};

// Get all assignments with filters
export function useAssignments(params?: { classId?: string; page?: number; limit?: number }) {
  return useQuery({
...
export function useCreateAssignment() {
  ...
  return useMutation({
    mutationFn: async (data: { classId: string; title: string; description?: string; dueDate: string; maxScore?: number }) => {
    ...
    onSuccess: (data) => {
      ...
      if (data.classId) {
        queryClient.invalidateQueries({ queryKey: assignmentKeys.byClass(data.classId) });
      }
    },
  });
}
...
export function useSubmitAssignment() {
  ...
  return useMutation({
    mutationFn: async ({ assignmentId, content, attachments }: { assignmentId: string; content?: string; attachments?: string[] }) => {
      const response = await api.post<Submission>(`/assignments/${assignmentId}/submissions`, { content, attachments });
      return response.data;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: assignmentKeys.submissions(data.assignmentId) });
    },
  });
}
...
export function useBulkGrade() {
  ...
  return useMutation({
    mutationFn: async ({ assignmentId, grades }: { assignmentId: string; grades: Array<{ submissionId: string; score: number; feedback?: string }> }) => {
