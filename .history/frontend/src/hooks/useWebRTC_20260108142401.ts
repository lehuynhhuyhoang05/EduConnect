...
export function useWebRTC(config: WebRTCConfig) {
  ...
  const createPeerConnection = useCallback((peerId: string): RTCPeerConnection => {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        socketManager.sendIceCandidate(parseInt(peerId), event.candidate);
      }
    };
  ...
  const createOffer = useCallback(async (peerId: string) => {
    ...
    try {
      ...
      socketManager.sendOffer(parseInt(peerId), offer);
    } catch (error) {
  ...
  const handleOffer = useCallback(async (peerId: string, offer: RTCSessionDescriptionInit) => {
    ...
    try {
      ...
      socketManager.sendAnswer(parseInt(peerId), answer);
    } catch (error) {
  ...
  const toggleAudio = useCallback(() => {
    if (localStreamRef.current) {
      ...
      if (audioTrack) {
        ...
        socketManager.emitMediaStateChange(sessionId, { video: isVideoEnabled, audio: audioTrack.enabled });
      }
    }
  }, [sessionId, isVideoEnabled]);
  ...
  const toggleVideo = useCallback(() => {
    if (localStreamRef.current) {
      ...
      if (videoTrack) {
        ...
        socketManager.emitMediaStateChange(sessionId, { video: videoTrack.enabled, audio: isAudioEnabled });
      }
    }
  ...
  useEffect(() => {
    ...
    const handleUserJoined = (data: { peerId: string }) => {
